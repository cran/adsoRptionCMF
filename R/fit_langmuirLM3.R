...
#' @name fit_langmuirLM3
#'
#' @description Performs linear modeling for the Langmuir adsorption isotherm using the linearized form Qe = Qmax - (1/b)(Qe/Ce).
#'
...
#'
#' @return A list containing the results of the linear Langmuir (Form 3) model fitting, including:
#' \itemize{
...
utils::globalVariables(c("Qe", "Qe_by_Ce", "Fit", "CI_lower", "CI_upper", "Fitted", "Residuals"))

fit_langmuirLM3 <- function(Ce, Qe, verbose = TRUE) {

  # Basic validation
  if (any(Ce <= 0) || any(Qe <= 0)) stop("Ce and Qe must be positive for Form 3 transformation.")

    Qe_by_Ce <- Qe / Ce
    data <- data.frame(Qe = Qe, Qe_by_Ce = Qe_by_Ce)

    # Fit linear model: Qe = Qmax - (1/b)(Qe/Ce)
    fit <- stats::lm(Qe ~ Qe_by_Ce, data = data)
    ...
    boot_fun <- function(data, indices) {
      d <- data[indices, ]
      tryCatch({
        fit_boot <- stats::lm(Qe ~ Qe_by_Ce, data = d)
        predict(fit_boot, newdata = data.frame(Qe_by_Ce = sort(data$Qe_by_Ce)))
    ...
    plot_data <- data.frame(
      Qe_by_Ce = sort(data$Qe_by_Ce),
      Fit = predict(fit, newdata = data.frame(Qe_by_Ce = sort(data$Qe_by_Ce))),
      CI_lower = ci_bounds[1, ],
      CI_upper = ci_bounds[2, ]
    )

    # Plot: Linear Fit with bootstrapped CI
    model_fit_plot <- ggplot2::ggplot(data, ggplot2::aes(x = Qe_by_Ce, y = Qe)) +
      ggplot2::geom_point(color = "blue", size = 2) +
      ggplot2::geom_line(data = plot_data, ggplot2::aes(x = Qe_by_Ce, y = Fit), color = "red", linewidth = 1.2) +
    ...
    residual_plot <- ggplot2::ggplot(data.frame(Fitted = fitted_vals, Residuals = residuals),
      ...
      ggplot2::annotate("text", x = max(fitted_vals) * 0.9, y = max(residuals) * 0.9,
                        ...
                        hjust = 1, size = 4) +
      ggplot2::labs(title = "Residual Plot for Langmuir Isotherm Third Linear Form",
    ...
    # Output if verbose
    if (verbose) {
      cat("=== Langmuir Isotherm Third Linear Model Summary ===\n")
      print(summary_fit)
