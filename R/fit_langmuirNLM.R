...
#' @name fit_langmuirNLM
#'
#' @description Performs non-linear modeling for the Langmuir adsorption isotherm.
#'
...
#'
#' @return A list containing the results of the  non-linear Langmuir model fitting, including:
#' \itemize{
...
utils::globalVariables(c("logQmax", "logKl", "log_sigma", "Fit", "CI_lower", "CI_upper", "Fitted", "Residuals"))

fit_langmuirNLM <- function(Ce, Qe) {

  # Prepare data
  data <- data.frame(Ce = Ce, Qe = Qe)

  # Fit Langmuir model
  model_formula <- Qe ~ (Qmax * Kl * Ce) / (1 + Kl * Ce)
  start_vals <- data.frame(Qmax = 1, Kl = 1)
  ...
  # Model summary
  cat("=== Langmuir Isotherm Non-linear Model Summary ===\n")
  ...
  residual_plot <- ggplot2::ggplot(data.frame(Fitted = fitted_vals, Residuals = residuals),
    ...
    ggplot2::labs(title = "Residual Plot for Langmuir Isotherm Non-linear Form",
                  x = "Fitted Values",
                  y = "Residuals") +
    ggplot2::theme(
      plot.title = ggplot2::element_text(hjust = 0.5, face = "bold"),
